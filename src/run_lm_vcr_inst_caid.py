        ...
        for j in range(len(lines)):
            ...
            if output:
                ...
                if len(output.strip()) == 1 and output in ['1','2','3','4']:
                    return ['1','2','3','4'].index(output.strip())
...
if __name__ == '__main__':
    ...
    data_path = '/mnt/user7/Main/VisualReasoning/results/captions/captions_blip2-flan-t5-xxl_vcr_fin_caid.jsonl'
    data = []
    ...
    reasoning_output_path = os.path.join(reasoning_path, f'{model_id_}_reasoning.txt')
    ...
    for j,d in enumerate(tqdm(data)):
        ...
        messages_q2a  = [
                            ...
                            {"role": "user", "content": input_q2a},
                        ]
        messages_qa2r  = [
                            {"role": "system", "content": SystemEvaluatePrompt_rat},
                            {"role": "user", "content": input_qa2r},
                        ]
        
        tokenized_chat_q2a = tokenizer.apply_chat_template(messages_q2a, tokenize=True, 
                                                           add_generation_prompt=True, 
                                                            return_tensors="pt", return_dict=True).to(device)
        tokenized_chat_qa2r = tokenizer.apply_chat_template(messages_qa2r, tokenize=True, 
                                                            add_generation_prompt=True, 
                                                            return_tensors="pt", return_dict=True).to(device)

        output_q2a = model.generate(**tokenized_chat_q2a, pad_token_id=tokenizer.eos_token_id, 
                                    max_new_tokens=64)
        output_qa2r = model.generate(**tokenized_chat_qa2r, pad_token_id=tokenizer.eos_token_id, 
                                    max_new_tokens=64)

        response_q2a = tokenizer.decode(output_q2a[0], skip_special_tokens=True)
        response_qa2r = tokenizer.decode(output_qa2r[0], skip_special_tokens=True)
